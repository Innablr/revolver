...
class RevolverConfig {
    ...
    validateConfig(data) {
        ...
        this.logger.debug('Read Revolver config: %j', config);
        ...
        this.logger.debug('Final Revolver config: %j', config);
        return config;
    }
    ...
    async getOrganisationsAccounts(creds) {
        const orgsRegion = 'us-east-1';
        const allAccounts = await Promise.all(
            creds.map(async cr => {
                const client = new AWS.Organizations({credentials: cr, region: orgsRegion});
                const accounts = await common.paginateAwsCall(client.listAccounts.bind(client), 'Accounts');
        ...
        this.logger.info('%d Accounts found on the Organizations listed', flatAccounts.length);
        return flatAccounts;
    }
