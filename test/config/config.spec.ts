...
const ORG_ACCOUNTS = [
  {
    ...
    settings: {
      region: 'ap-southeast-1', // explicitly included
    },
  },
  {
...
describe('Validate test config', function () {
  it('Check simple parsing', async function () {
    ...
    expect(config.defaults.settings.auditLog?.json?.sns?.url).to.equal('arn:something:sns:topic');
    ...
    expect(config.organizations[0].settings.region).to.equal('ap-southeast-2');
